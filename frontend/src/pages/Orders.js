      ...
      <div className="relative bg-white flex flex-col md:flex-row w-full max-w-5xl mx-auto my-2 md:my-4 rounded-2xl overflow-hidden border border-sky-100 animate-fade-in max-h-[calc(100dvh-1rem)] md:max-h-[calc(100vh-2rem)]"
        ...
        <div className={`flex-col md:w-80 md:min-w-[20rem] md:flex-shrink-0 bg-slate-50 md:border-t-0 ${showMobileCart ? 'flex flex-1 min-h-0 w-full' : 'hidden md:flex'}`}>
          ...
          <div className="p-4 border-b border-slate-200 flex items-center gap-3 bg-white">
            <button onClick={() => setShowMobileCart(false)} className="md:hidden btn btn-ghost btn-icon">
              <ArrowLeftIcon className="h-5 w-5" />
            </button>
            <ShoppingCartIcon className="h-5 w-5 text-sky-500" />
            <h2 className="font-black text-slate-800 flex-1">
              Cart <span className="text-sky-500">({totalQty})</span>
            </h2>
            {cart.length > 0 && (
              <span className="text-sm font-black text-sky-600">৳{total.toFixed(0)}</span>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {cart.length === 0 ? (
              <div className="text-center mt-12">
                ...
                <p className="text-slate-300 text-xs mt-1">Go back and add items from the menu</p>
              </div>
            ) : cart.map(c => (
              <div key={c.id} className="bg-white rounded-xl border border-slate-100 p-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-bold text-slate-800 truncate">{c.name}</div>
                  <div className="text-xs text-slate-400 mt-0.5">
                    ৳{parseFloat(c.price).toFixed(0)} × {c.qty} = <span className="font-semibold text-slate-600">৳{(parseFloat(c.price) * c.qty).toFixed(0)}</span>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <button type="button" onClick={() => changeQty(c.id, -1)}
                    className="h-8 w-8 rounded-lg bg-slate-100 flex items-center justify-center hover:bg-slate-200 active:bg-slate-300">
                    <MinusIcon className="h-3.5 w-3.5 text-slate-600" />
                  </button>
                  <span className="w-7 text-center text-sm font-black text-slate-800">{c.qty}</span>
                  <button type="button" onClick={() => changeQty(c.id, 1)}
                    className="h-8 w-8 rounded-lg bg-sky-100 flex items-center justify-center hover:bg-sky-200 active:bg-sky-300">
                    <PlusIcon className="h-3.5 w-3.5 text-sky-600" />
                  </button>
                  <button type="button" onClick={() => setCart(p => p.filter(x => x.id !== c.id))}
                    className="h-8 w-8 rounded-lg flex items-center justify-center text-rose-400 hover:bg-rose-50 active:bg-rose-100 ml-0.5">
                    <TrashIcon className="h-3.5 w-3.5" />
                  </button>
                </div>
              </div>
          ...
          <div className="p-4 border-t border-slate-200 space-y-3 bg-white pb-[calc(1rem+env(safe-area-inset-bottom))]">
            ...
            <button
              ...
              className="btn btn-primary w-full disabled:opacity-50 justify-center text-base py-3">
              {submitting ? <LoadingSpinner size="sm" /> : <><CheckIcon className="h-5 w-5" />Place Order</>}
            </button>
          </div>
        </div>
